        ...
        switch (state) {
            ...
            case State.READ_COLON:
                switch (char) {
                    ...
                    case 'n':
                        return new NextState(State.READ_N, '');
                    case '\\':
                        return new NextState(State.READ_SLASH, ':');
                    default:
                        return new NextState(State.READ_CHAR, `:${char}`);
            ...
            case State.READ_N:
                switch (char) {
                    case 'o':
                        return new NextState(State.READ_NO, '');
                    case '\\':
                        return new NextState(State.READ_SLASH, ':n');
                    default:
                        return new NextState(State.READ_CHAR, `:n${char}`);
                }
            case State.READ_NO:
                switch (char) {
                    case 'd':
                        return new NextState(State.READ_NOD, '');
                    case '\\':
                        return new NextState(State.READ_SLASH, ':no');
                    default:
                        return new NextState(State.READ_CHAR, `:no${char}`);
                }
            case State.READ_NOD:
                switch (char) {
                    case 'e':
                        return new NextState(State.READ_CHAR, 'node');
                    case '\\':
                        return new NextState(State.READ_SLASH, ':nod');
                    default:
                        return new NextState(State.READ_CHAR, `:nod${char}`);
                }
